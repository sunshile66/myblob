...
public interface PostRepository extends JpaRepository<Post, Long> {
       ...
       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.status = 'PUBLISHED' AND p.deleted = false ORDER BY p.top DESC, p.publishedAt DESC")
       Page<Post> findPublishedPosts(Pageable pageable);

       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.status = 'PUBLISHED' AND p.deleted = false AND " +
                     "(LOWER(p.title) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
                     "LOWER(p.content) LIKE LOWER(CONCAT('%', :search, '%')))")
       Page<Post> searchPosts(@Param("search") String search, Pageable pageable);

       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.category.slug = :categorySlug AND p.status = 'PUBLISHED' AND p.deleted = false " +
       ...
       @Query("SELECT p FROM Post p JOIN p.tags t LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "WHERE t.slug = :tagSlug AND p.status = 'PUBLISHED' AND p.deleted = false ORDER BY p.publishedAt DESC")
       Page<Post> findByTagSlug(@Param("tagSlug") String tagSlug, Pageable pageable);

       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.author.id = :authorId AND p.deleted = false ORDER BY p.createdAt DESC")
       Page<Post> findByAuthorId(@Param("authorId") Long authorId, Pageable pageable);

       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.author.id = :authorId AND p.status = :status AND p.deleted = false ORDER BY p.createdAt DESC")
        ...
        */
       @Query("SELECT p FROM Post p LEFT JOIN FETCH p.author LEFT JOIN FETCH p.category " +
                     "LEFT JOIN FETCH p.tags WHERE p.status = :status AND p.deleted = false ORDER BY p.publishedAt DESC")
